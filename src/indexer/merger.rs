...
impl IndexMerger {
    ...
    fn write_spatial_fields(
        ...
        // Build doc ID mapping (same as before)
        let mut segment_mappings: Vec<Vec<Option<DocId>>> = Vec::new();
        for reader in &self.readers {
            segment_mappings.push(vec![None; reader.max_doc() as usize]);
        }
        for (new_doc_id, old_doc_addr) in doc_id_mapping.iter_old_doc_addrs().enumerate() {
        ...
        for (field, field_entry) in self.schema.fields() {
            if matches!(field_entry.field_type(), FieldType::Spatial(_)) {
                    ...
                    for envelope_result in LeafPageIterator::new(&segment) {
                        for envelope in envelope_result? {
                            if let Some(new_doc_id) =
                                segment_mappings[segment_ord][envelope.doc_id as usize]
                            {
                ...
                spatial_serializer.serialize_field(field, &mut envelopes)?;
            }
        }
