...
use super::exact::expensive_sign;
...
pub fn sign_with_cross(a: &[f64; 3], b: &[f64; 3], c: &[f64; 3], a_cross_b: &[f64; 3]) -> i32 {
    ...
    // Try StableSign before falling back to exact arithmetic.
    let result = stable_sign(a, b, c);
    if result != 0 {
        return result;
    }
...
fn ortho(a: &[f64; 3]) -> [f64; 3] {
    ...
    // Use axis of largest component to avoid numerical issues
...
fn symbolic_cross_prod_sorted(a: &[f64; 3], b: &[f64; 3]) -> [f64; 3] {
    debug_assert!(a < b);
    // debug_assert!(exact cross product a x b is zero) - omitted, would require exact arithmetic
...
///
/// StableSign in s2predicates.cc, lines 51-102
...
impl S2EdgeCrosser {
    ...
    fn crossing_sign_internal(&mut self, d: &[f64; 3]) -> i32 {
        ...
        if self.acb == 0 {
            self.acb = -sign(&self.a, &self.b, &self.c);
        }
        debug_assert!(self.acb != 0);

        if self.bda == 0 {
            self.bda = sign(&self.a, &self.b, d);
        }
