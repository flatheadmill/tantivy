...
pub fn clip_to_face(a: &[f64; 3], b: &[f64; 3], face: u8) -> Option<([f64; 2], [f64; 2])> {
...
pub fn clip_to_padded_face(
    ...
    // Convert to (u,v,w) coordinates of this face.
...
/// Returns true if the line (defined by its normal n) intersects the face.
...
fn get_exit_axis(n: &[f64; 3]) -> u8 {
    ...
    if intersects_opposite_edges(n) {
        ...
        let sign = |x: f64| x.is_sign_negative() as u8;
...
fn get_exit_point(n: &[f64; 3], axis: u8) -> [f64; 2] {
    if axis == 0 {
