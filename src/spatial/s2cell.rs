...
impl S2Cell {
    ...
    pub fn get_size_ij(&self) -> i32 {
        S2CellId::size_ij_for_level(self.level)
    }
    ...
    pub fn get_vertex_raw(&self, k: usize) -> [f64; 3] {
        ...
        face_uv_to_xyz(self.face, v[0], v[1])
    }
    ...
    pub fn get_edge_raw(&self, k: usize) -> [f64; 3] {
        match k & 3 {
            0 => get_v_norm(self.face, self.uv[1].lo()),
            1 => get_u_norm(self.face, self.uv[0].hi()),
            2 => neg(get_v_norm(self.face, self.uv[1].hi())),
            _ => neg(get_u_norm(self.face, self.uv[0].lo())),
        }
    }
    ...
    pub fn subdivide(&self) -> Option<[S2Cell; 4]> {
        ...
        for (pos, child_id) in child_ids.iter().enumerate() {
            let ij = s2coords::pos_to_ij(self.orientation, pos as u8);
            let i = (ij >> 1) as usize;
            let j = (ij & 1) as usize;
            ...
            children[pos] = S2Cell {
                ...
                face: self.face,
                level: self.level + 1,
                orientation: self.orientation ^ s2coords::pos_to_orientation(pos as u8),
                uv: R2Rect::new(
                    R1Interval::new(
    ...
    pub fn contains_point(&self, p: &[f64; 3]) -> bool {
        ...
        if let Some(uv) = s2coords::face_xyz_to_uv(self.face, p) {
